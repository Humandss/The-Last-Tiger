...
public class CameraController : MonoBehaviour
{
    ...
    [Header("Hit Shake")]
    [SerializeField] private Vector3 maxHitKickEuler = new Vector3(8f, 6f, 4f); // 발사보다 크게
    [SerializeField] private float hitKickInSpeed = 25f;   // 발사보다 느리게 (묵직함)
    [SerializeField] private float hitReturnSpeed = 4f;    // 천천히 복귀
    ...
    public void TriggerHitShake(Vector3 hitDirection, float intensity = 1f)
    {
        ...
        Vector3 kick = new Vector3(
            maxHitKickEuler.x * intensity,                              // Pitch 항상 위로
            maxHitKickEuler.y * intensity * yawSign * Random.Range(0.5f, 1f),  // Yaw 랜덤
            maxHitKickEuler.z * intensity * rollSign * Random.Range(0.3f, 1f)  // Roll 랜덤
        );
